...
class GroupRepository(BaseRepository):
    ...
    def find(
        ...
        with closing(self.db_session.cursor()) as db_cursor:
            for groupname in groupnames:
                ...
                groups.append(Group(groupname=groupname, checks=checks, replies=replies))
    ...
    def count_users(self, groupname: str) -> int:
        with closing(self.db_session.cursor()) as db_cursor:
            sql = f"SELECT COUNT(DISTINCT username) FROM {self.rad_tables.radusergroup} WHERE groupname = {self.ph}"
            db_cursor.execute(sql, (groupname,))
            (count,) = db_cursor.fetchone()
...
class HuntGroupRepository(BaseRepository):
    def find_one(self, id: int) -> HuntGroup | None:
        with closing(self.db_session.cursor()) as db_cursor:
            sql = (
                f"SELECT id, groupname, nasipaddress, nasportid "
                f"FROM {self.rad_tables.radhuntgroup} WHERE id = {self.ph}"
            )
            ...
            if not row:
                return None
            i, g, n_ip, n_port = row
            return HuntGroup(id=i, groupname=g, nasipaddress=n_ip, nasportid=n_port or "")

    def find(
        self,
        limit: int | None = 100,
        groupname_like: str | None = None,
        ...
        where_clauses_as_text = f"WHERE {' AND '.join(where_clauses)}" if where_clauses else ""
        sql = (
            "SELECT id, groupname, nasipaddress, nasportid "
            f"FROM {self.rad_tables.radhuntgroup} "
            f"{where_clauses_as_text} "
        ...
        with closing(self.db_session.cursor()) as db_cursor:
            ...
            return [
                HuntGroup(id=i, groupname=g, nasipaddress=n_ip, nasportid=n_port or "") for i, g, n_ip, n_port in rows
            ]
    ...
    def set(
        ...
        new_nasipaddress: str | None = None,
        new_nasportid: str | None = None,
    ):
        with closing(self.db_session.cursor()) as db_cursor:
            if new_groupname is not None:
                sql = f"UPDATE {self.rad_tables.radhuntgroup} SET groupname = {self.ph} WHERE id = {self.ph}"
                db_cursor.execute(sql, (new_groupname, id))

            if new_nasipaddress is not None:
                sql = f"UPDATE {self.rad_tables.radhuntgroup} SET nasipaddress = {self.ph} WHERE id = {self.ph}"
                db_cursor.execute(sql, (new_nasipaddress, id))

            if new_nasportid is not None:
                sql = f"UPDATE {self.rad_tables.radhuntgroup} SET nasportid = {self.ph} WHERE id = {self.ph}"
                db_cursor.execute(sql, (new_nasportid, id))

    def add(self, groupname: str, nasipaddress: str, nasportid: str = "") -> HuntGroup:
        with closing(self.db_session.cursor()) as db_cursor:
            sql = (
                f"INSERT INTO {self.rad_tables.radhuntgroup} (groupname, nasipaddress, nasportid) "
                f"VALUES ({self.ph}, {self.ph}, {self.ph})"
            )
            db_cursor.execute(sql, (groupname, nasipaddress, nasportid))
            new_id = db_cursor.lastrowid
            return HuntGroup(id=new_id, groupname=groupname, nasipaddress=nasipaddress, nasportid=nasportid)

    def remove(self, id: int):
        with closing(self.db_session.cursor()) as db_cursor:
            sql = f"DELETE FROM {self.rad_tables.radhuntgroup} WHERE id = {self.ph}"
            db_cursor.execute(sql, (id,))

    def remove_by_groupname(self, groupname: str):
        with closing(self.db_session.cursor()) as db_cursor:
            sql = f"DELETE FROM {self.rad_tables.radhuntgroup} WHERE groupname = {self.ph}"
            db_cursor.execute(sql, (groupname,))

    def exists_by_groupname(self, groupname: str) -> bool:
        with closing(self.db_session.cursor()) as db_cursor:
            sql = f"SELECT COUNT(*) FROM {self.rad_tables.radhuntgroup} WHERE groupname = {self.ph}"
            db_cursor.execute(sql, (groupname,))
            (count,) = db_cursor.fetchone()
            return count > 0
        ...
        with closing(self.db_session.cursor()) as db_cursor:
            ...
            return [
                RadAcct(
                    ...
                    username=row[3] or "",
                    realm=row[4],
                    nasipaddress=row[5] or "",
                    nasportid=row[6],
                    nasporttype=row[7],
                    acctstarttime=row[8],
                    acctupdatetime=row[9],
                    acctstoptime=row[10],
                    acctinterval=row[11],
                    acctsessiontime=row[12],
                    acctauthentic=row[13],
                    connectinfo_start=row[14],
                    connectinfo_stop=row[15],
                    acctinputoctets=row[16],
                    acctoutputoctets=row[17],
                    calledstationid=row[18] or "",
                    callingstationid=row[19] or "",
                    acctterminatecause=row[20] or "",
                    servicetype=row[21],
                    framedprotocol=row[22],
                    framedipaddress=row[23] or "",
                    framedipv6address=row[24] or "",
                    framedipv6prefix=row[25] or "",
                    framedinterfaceid=row[26] or "",
                    delegatedipv6prefix=row[27] or "",
                    class_attr=row[28],
                )
                for row in rows
            ]
