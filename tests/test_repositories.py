...
from freeradius.models import AttributeOpValue, Group, GroupUser, HuntGroup, Nas, User, UserGroup
...
def test_huntgroup(repositories):
    huntgroup_name = "test_hg"
    nas_ip = "1.2.3.4"
    nas_port = "1"
    
    # adding
    hg = repositories.huntgroup.add(groupname=huntgroup_name, nasipaddress=nas_ip, nasportid=nas_port)
    assert hg.id > 0
    assert hg.groupname == huntgroup_name
    assert hg.nasipaddress == nas_ip
    assert hg.nasportid == nas_port
    
    # find_one
    hg_found = repositories.huntgroup.find_one(hg.id)
    assert hg_found == hg
    
    # exists_by_groupname
    assert repositories.huntgroup.exists_by_groupname(huntgroup_name)
    
    # set (update)
    new_name = "new_hg_name"
    repositories.huntgroup.set(id=hg.id, new_groupname=new_name)
    hg_updated = repositories.huntgroup.find_one(hg.id)
    assert hg_updated.groupname == new_name
    
    # find
    found_list = repositories.huntgroup.find(groupname_like="new_%")
    assert len(found_list) == 1
    assert found_list[0] == hg_updated
    
    # remove
    repositories.huntgroup.remove(hg.id)
    assert repositories.huntgroup.find_one(hg.id) is None
    
    # remove_by_groupname
    hg2 = repositories.huntgroup.add(groupname="hg2", nasipaddress="2.2.2.2")
    assert repositories.huntgroup.exists_by_groupname("hg2")
    repositories.huntgroup.remove_by_groupname("hg2")
    assert not repositories.huntgroup.exists_by_groupname("hg2")
